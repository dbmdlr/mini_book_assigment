...
public class RequestDAO
{
    public List<Request> getRequest_To(Integer request_to) {
        ...
        String getrequestById = "SELECT u.user_id, u.first_name, u.last_name, u.email, "
                ...
                + "WHERE r.REQUEST_TO = ? AND r.RESPONDED = 0 "
    ...
    public List<Integer> getAllRequested(Integer user_id) {
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet result = null;
        String allRequested = "SELECT request_to FROM REQUEST WHERE user_request = ? AND responded=0";
        List<Integer> rList = null;
        try
        {
            conn = OracleConnection.getConnection();
            stmt = conn.prepareStatement(allRequested);
            stmt.setInt(1, user_id);
            result = stmt.executeQuery();
            rList = new ArrayList<Integer>();
            
            while(result.next()) {
                rList.add(result.getInt(1));
            }
            return rList;
        }
        catch (ClassNotFoundException | IOException | SQLException e)
